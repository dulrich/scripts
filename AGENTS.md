...
| `util/cache-prune.sh` (`util cache-prune`) | Reports and prunes rebuildable language-runtime caches (uv, npm, docker build cache, pip, bun, cargo) for the invoking user. Refuses to run as root — every cache here lives under the calling account. Reports total footprint alongside estimated reclaimable, not one conflated figure: directory-backed runtimes use a hardlink-aware census (a cache entry also linked outside the tree counts toward total but not reclaimable); docker parses `system df` (primary) or `buildx du` (fallback) and its reclaimable figure is labelled an explicit upper bound; the safe docker prune runs unfiltered by default (the full build cache docker reports as reclaimable), with `--docker-until` as an opt-in age-window narrowing, not the default. `--report` shows sizes only; the safe verb (uv/npm/docker) runs interactively-confirmed or via `--yes`; `--yes --include-purge` purges pip/bun (their only verb) and, for uv/npm, purges instead of running their safe verb — `uv cache clean`/`npm cache clean --force` are destructive and force a re-download, unlike their conservative safe verbs; interactively each destructive purge gets its own confirm, unconditional for pip/bun and gated on `--include-purge` for uv/npm; cargo is report-only. See `-h` for the full mode matrix. |
