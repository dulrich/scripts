...
#   - Snyk, "Miasma Attack Hits Red Hat npm Packages"; Tenable, "Mini Shai-Hulud
#     FAQ" (TeamPCP) -- CVE-2026-45321 (TanStack)
# ============================================================================
...
# is high-signal. Executed via "bun run", evading node-only monitoring. Matched as
# a "*/<rel>" suffix over the P1 inventory: a new entry whose basename that walk
# does not already collect needs its name added in inventory.sh.
SETUP_FILES=(".claude/setup.mjs" ".vscode/setup.mjs"
  ".cursor/rules/setup.mdc" ".github/setup.js")
...
INJECT_CONFIGS=(".claude/settings.json" ".claude/settings.local.json"
  ".gemini/settings.json" ".cursor/settings.json")
...
# value and evades attribution (rule 6). This walks the JSON tracking brace depth
# and returns only a depth-1 key's value. It executes nothing in the file.
json_string_field() {
  ...
  perl -e '
    ...
    while ($i < $len) {
      ...
      if ($c eq "\"") {
        my $j = $i+1; my $key = "";
        while ($j < $len) { my $d = substr($s,$j,1);
          if ($d eq "\\") { $key .= substr($s,$j+1,1); $j+=2; next; }
          last if $d eq "\""; $key .= $d; $j++; }
        my $here = $depth; $i = $j+1;
        if ($here == 1) {
...
# The shared ladder (lib/policy.sh) with the npm predicates bound to it. The
# classification is RETURNED: the calling check owns its package_hits counter.
...
# npm/yarn/pnpm formats. CRITICAL (rule 5): for yarn and yarn-berry the entry KEY
# carries a semver RANGE (@x/y@^1.2.3), not the resolved version, so the resolved
# value is read off the block's own "version"/"resolution" line. Spurious pairs are
# harmless: report_package_reference only acts on known-bad/prefix/watch names.
scan_lock_pairs() {
  local lock="$1"
  ...
  perl -0777 -ne '
    ...
    while (/"node_modules\/((?:@[^\/"]+\/)?[^\/"]+)"\s*:\s*\{.*?"version"\s*:\s*"([^"]+)"/sg) { print "$1\t$2\n"; }
    # npm package-lock v1 (and nested deps): "<name>": { "version": "x.y.z" ... }
    while (/"((?:@[^"\/]+\/)?[^"\/@]+)"\s*:\s*\{\s*"version"\s*:\s*"([^"]+)"/g) { print "$1\t$2\n"; }
    ...
    while (/^[ \t]*"?((?:@[^\/\s"]+\/)?[^@\s",]+)@[^\n:]*:[ \t]*\r?\n(?:[^\n]*\n)*?[ \t]+version:?[ \t]+"?([^"\s]+)"?/mg) { print "$1\t$2\n"; }
    # pnpm v6-v8: "  /<name>@<version>:"  (leading slash; optional (peer) suffix)
    while (/^[ \t]{2,}\/((?:@[^\/\s:]+\/)?[^@\s:]+)@([^:\s(]+)[:(]/mg) { print "$1\t$2\n"; }
    # pnpm v9: "  \x27<name>@<version>\x27:"  (quoted, NO leading slash)
    while (/^[ \t]+\x27((?:@[^\/\s\x27]+\/)?[^@\s\x27]+)@([^\x27\s(]+)\x27:/mg) { print "$1\t$2\n"; }
    # yarn-berry resolution lines: resolution: "<name>@npm:<version>"
    while (/resolution:\s*"((?:@[^\/"]+\/)?[^@"]+)@(?:npm:)?([^"(]+?)(?:\([^"]*\))?"/g) { print "$1\t$2\n"; }
  ' "$lock" 2>/dev/null
}

npm_check_installed_packages() {
  local pkgjson modpkg name version projects_found=0 package_hits=0
  section "npm: affected package families (installed trees)"
  while IFS= read -r -d '' pkgjson; do
    [ -d "$(dirname "$pkgjson")/node_modules" ] || continue
    projects_found=$((projects_found+1))
  done < <(inventory_root | inventory_select any 'package.json')
  ...
  done < <(inventory_modules | inventory_select any '*/node_modules/*/package.json')
  info "$projects_found npm project(s) found"
  info "$package_hits affected-package match(es) found"
}

npm_check_phantom_gyp() {
  local bg rootidx bad nonidiom sz binding_gyp_hits=0 gyp_total=0
  ...
  # NOTE: binding.gyp is NORMAL for native modules, and "<!(node -p \"require(
  # 'node-addon-api').include_dir\")" is the legitimate idiom -- node -p on an
  # EXPRESSION, not a .js file. See AGENTS.md FP rule 1.
  while IFS= read -r -d '' bg; do
    gyp_total=$((gyp_total+1))
    rootidx="$(dirname "$bg")/index.js"
    bad=0
    if [ -f "$rootidx" ] && grep -Eq "$PAYLOAD_MARKERS" "$rootidx" 2>/dev/null; then
      ...
      if [ -n "$nonidiom" ]; then
        ...
        sz="$([ -f "$rootidx" ] && wc -c < "$rootidx" 2>/dev/null | tr -d ' ')"
        [ -n "${sz:-}" ] && info "    (root index.js is ${sz} bytes)"
      fi
    fi
    binding_gyp_hits=$((binding_gyp_hits+bad))
  done < <(inventory_modules | inventory_select any '*/node_modules/*/binding.gyp')
  info "$gyp_total binding.gyp file(s) seen; $binding_gyp_hits weaponized"
}

npm_check_lockfiles() {
  local lock name version scopes scope
  ...
  while IFS= read -r -d '' lock; do
    ...
    report_markers hit "affected package reference in $lock" "$lock" "$PKG_RE" 40
    grep -Eq "$WATCH_RE" "$lock" 2>/dev/null || continue
  ...
  done < <(inventory_root | inventory_select any 'package-lock.json' 'npm-shrinkwrap.json' 'yarn.lock' 'pnpm-lock.yaml')
}

npm_check_injected_files() {
  local rel f
  section "npm: injected AI-assistant / editor backdoor files"
  for rel in "${SETUP_FILES[@]}"; do
    while IFS= read -r -d '' f; do
      hit "injected setup file ($rel): $f"
      show_matches "$f" "$PAYLOAD_MARKERS|$IOC_RE|bun run" 10
    done < <(inventory_root | inventory_select any "*/$rel")
  done
  for rel in "${INJECT_CONFIGS[@]}"; do
    while IFS= read -r -d '' f; do
      report_markers hit "malicious content injected into config ($rel): $f" "$f" \
        "$PAYLOAD_MARKERS|$IOC_RE|setup\.mjs|bun run" 10
    done < <(inventory_root | inventory_select any "*/$rel")
  done
}

npm_check_agent_persistence() {
  local ROOT="$1" cfg d f
  section "npm: Claude Code / OpenCode persistence (SessionStart hooks)"
  local agent_config_files=("$HOME/.claude/settings.json" "$HOME/.claude/settings.local.json"
    "$ROOT/.claude/settings.json" "$ROOT/.opencode/opencode.json" "$ROOT/.opencode/config.json")
  while IFS= read -r -d '' cfg; do
    if grep -Eq "$IOC_RE|setup\.mjs|bun run" "$cfg" 2>/dev/null; then
      hit "malicious agent persistence signature: $cfg"
      show_matches "$cfg" "$IOC_RE|setup\.mjs|bun run|SessionStart" 0
    elif grep -q 'SessionStart' "$cfg" 2>/dev/null; then
      review "SessionStart hook present (verify the command is yours): $cfg"
      show_matches "$cfg" 'SessionStart|command' 20
    fi
  done < <(dedupe_existing_files "${agent_config_files[@]}")
  # P3: the agent-config dirs are their own scope (own roots, own prune list).
  for d in "$ROOT/.claude" "$ROOT/.opencode" "$HOME/.claude" "$HOME/.cursor" "$HOME/.gemini"; do
    [ -d "$d" ] || continue
    while IFS= read -r -d '' f; do
      report_markers hit "agent-config IOC: $f" "$f" "$IOC_RE|$PAYLOAD_MARKERS" 0
    done < <(find "$d" \
...
npm_check_vscode_tasks() {
  local task
  section "npm: VS Code task persistence (folderOpen auto-run)"
  while IFS= read -r -d '' task; do
    grep -Eq 'folderOpen' "$task" 2>/dev/null || continue
    hit "folderOpen task persistence: $task"
    show_matches "$task" 'folderOpen|setup\.mjs|bun' 0
  done < <(inventory_root | inventory_select any '*/.vscode/tasks.json')
}

npm_check_source_markers() {
  local f
  section "npm: payload code markers in source / config trees"
  while IFS= read -r -d '' f; do
    report_markers hit "payload/IOC marker: $f" "$f" "$PAYLOAD_MARKERS|$IOC_RE" 5
  done < <(inventory_root | inventory_select file '*.js' '*.mjs' '*.cjs' 'binding.gyp')
}

run_npm_checks() {
  local ROOT="$1"
  inventory_build_modules "$ROOT"
  npm_check_installed_packages
  npm_check_phantom_gyp
  npm_check_lockfiles
  npm_check_injected_files
  npm_check_agent_persistence "$ROOT"
  npm_check_vscode_tasks
  npm_check_source_markers
}
